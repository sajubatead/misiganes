...
require_once ( $dir . 'apartments-manager-inputs.php');
require_once ( $dir . 'apartments-manager-custompost.php');
require_once ( $dir . 'apartments-manager-flats.php');
require_once ( $dir . 'apartments-manager-quick-edit.php');

function um_apartments_manager_admin_scripts() {  //Import Plugin Scripts
global $pagenow, $typenow; //get the post type

if( $typenow == 'apartments_listing'){

  wp_enqueue_style('um-admin-css', plugins_url('css/admin-apartments.css', __FILE__) );// unique name, and source, and dependency

}

if( ($pagenow == 'post.php' || $pagenow= 'post-new.php') && $typenow == 'apartments_listing' ) { //if pages exist(post edit screen, post type screen) then on those screens load plugin's scripts
  ...
  if( $pagenow == 'post-new.php' && $typenow == 'apartments_listing'){  ///////////////////////////////////////////post-new??//////////////
...
add_action('admin_enqueue_scripts','um_apartments_manager_admin_scripts');
